/**
 * ============================================================
 * REST API Plugin - Candidates Routes
 * Endpoints for managing HR candidates via REST API
 * Uses the candidates table which has userId for ownership
 * ============================================================
 */
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedApiRequest, res: Response) => {
    ...
    const [existing] = await db
      .select()
      .from(candidates)
      .where(and(eq(candidates.userId, userId), eq(candidates.phone, data.phone!)))
      .limit(1);
    ...
    for (let i = 0; i < newContacts.length; i++) {
      ...
      try {
        const [existing] = await db
          .select()
          .from(candidates)
          .where(and(eq(candidates.userId, userId), eq(candidates.phone, contact.phone)))
          .limit(1);
